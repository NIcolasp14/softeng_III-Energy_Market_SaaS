...
exports.postSystem = (req, res) => {
    ...
    try {
        ...
        let sessions=[];
        //send start updating to channel
        ...
        fs.createReadStream(path)
            ...
            .on("data", (row) => {
                ...
                if (row.DateTime == '' || row.ResolutionCode == '' ||
                    ...
                    let row1;
                    //creation of record for database
                    row1={
                        datetime: row.DateTime,
                        actualgeneration: row.ActualGenerationOutput,
                        actualcon: row.ActualConsumption,
            ...
            .on("end", () => {
                ...
                //inform that data send to agpt ms ended
                ...
                console.log("end--------------------")
                //return;
                return res.status(200).send(`import agpt data`);
            });
    } catch (error) {
        console.log(error);
        res.status(500).send({
